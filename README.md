...
Then use a skill directly with `/viky-gen-ai-factory:indian-law-factory` or `/viky-gen-ai-factory:tamil-language-factory`, or just ask — both trigger automatically on the right kind of question.
...
`claude plugin list` shows it enabled with both skills in the component
inventory (`claude plugin details viky-gen-ai-factory`).
...
`tamil-language-factory` is a language-learning and reference aid, not an
academic or scholarly source — disputed historical dates and details are
flagged as disputed, not settled, in its own `REFERENCES.md`.
